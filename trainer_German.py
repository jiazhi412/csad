...
class Trainer(object):
    def __init__(self, option):
        ...
        self.German_train_mode = option.German_train_mode
        self.German_test_mode = option.German_test_mode
        # self.save_dir = os.path.join(option.save_dir, option.exp_name, option.attributes[0], option.eval_mode)
        self.save_dir = os.path.join(option.save_dir, option.exp_name, option.German_train_mode, option.German_test_mode)
        ...
        if self.German_train_mode.endswith('ex'):
    ...
    def _save_model(self, step, prefix=''):
        filename = os.path.join(self.option.save_dir, self.option.exp_name, self.option.German_train_mode, self.option.German_test_mode, prefix+'checkpoint_step_%04d.pth' % step)
        torch.save({
            'step': step,
    ...
    def train(self, train_loader, val_loader=None):
        ...
        data = {
            ...
            'Train': [self.German_train_mode],
            'Test': [self.German_test_mode],
            'CSAD': [final_acc * 100]
            }
        utils.append_data_to_csv(data, os.path.join(self.exp_dir, 'German_CSAD_trials.csv'))
