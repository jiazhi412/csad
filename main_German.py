...
import os
import random
from trainer_German import Trainer
from utils import save_option_German
import argparse
from dataloader.German import GermanDataset 
import utils
import h5py
# import warnings
...
def backend_setting(option):
    log_dir = os.path.join(option.save_dir, option.exp_name, option.German_train_mode, option.German_test_mode)
...
def main():
    ...
    ## German
    parser.add_argument("--German_train_mode", type=str, default='eb1')
    parser.add_argument("--German_test_mode", type=str, default='unbiased')

    torch.set_num_threads(1)
    option = parser.parse_args()
    # extreme bias n_class_bias
    if option.German_train_mode.endswith('ex'):
    ...
    data_folder = '/nas/vista-ssd01/users/jiazli/datasets/German/German_newData.csv'

    

    train_set = GermanDataset(data_folder, option.German_train_mode, quick_load=True, bias_name=option.bias_name)
    dev_set = GermanDataset(data_folder, option.German_test_mode, quick_load=True, bias_name=option.bias_name)
    test_set = GermanDataset(data_folder, option.German_test_mode, quick_load=True, bias_name=option.bias_name)
