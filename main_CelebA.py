...
def main():
    ...
    # self.train_target = np.array([target_dict[key] for key in train_key_list])
    ...
    trainval_loader = torch.utils.data.DataLoader(train_set, batch_size=option.batch_size, shuffle=True, num_workers=4)
    # dev_loader = torch.utils.data.DataLoader(dev_set, batch_size=option.batch_size, shuffle=True, num_workers=4)
    testloader = torch.utils.data.DataLoader(test_set, batch_size=option.batch_size, shuffle=True, num_workers=4)
